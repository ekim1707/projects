...
class Person:
    ...
    def __init__(self, name, email, phone, greeting_count): 
        self.name = name 
        self.email = email 
        self.phone = phone
    ...
    def greet(self, other_person):
        ...
        print('Hello {}, I am {}!'.format(other_person.name, self.name))
        if other_person not in self.friends == True:
            self.greeting_count += 1
    def print_contact_info(self):
        print(self.email, self.phone)
    def add_friends(self, friend):
        if friend not in self.friends == True:
...
myCar = Vehicle('Lexus', 'CT2000', 2015)
myCar.print_info()
